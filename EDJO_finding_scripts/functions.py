...
def low_pass_filter(cube,window,days):
    
    dwgt=low_pass_weights(window, 1. / days )
    cube=cube.rolling_window('time',iris.analysis.SUM,len(dwgt),weights=dwgt)
    
    return cube

def low_pass_weights(window, cutoff): 
    """
    Taken from: https://scitools.org.uk/iris/docs/latest/examples/General/SOI_filtering.html
...
def blob_finder(wind,Ucrit,gridarea,region_length,zonal_length):
    ...
    for day in days: 
        ...
        if Ucrit =='daily mean': 
            Ucrit = np.mean(wind_intense[wind_intense>0])
        ...
        for maxima in region_maxima_coords: 
            ...
            if max_vel == -1.0:
                ...
                regionEDJ= EDJregion(region[0],flood_max,wind_intense,gridarea.data,lon,lat)
